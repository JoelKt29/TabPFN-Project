...
from step08_stacking_train import TabPFNStackingModel
...
def generate_aligned_3d_proof():
    print("--- Generating 3D proof (statistical alignment) ---")
    ...
    # 2. Context TabPFN
    df_train = pd.read_csv(data_dir / "sabr_adaptive_dataset.csv").sample(5000)
    ...
    # 4. Compute the ground truth BEFORE anything else
    print("Computing the mathematical surface...")
    ...
    with torch.no_grad():
        ...
        # Raw network output (roughly normalized to [-1, 1])
        raw_ai_preds = hybrid_out[:, 6].cpu().numpy()

    # 6. STATISTICAL ALIGNMENT (core of the fix)
    # Instead of the broken JSON scaler, align the model distribution onto the true one
    ...
    print(f"Ground-truth stats -> mean: {true_mean:.4f}, std: {true_std:.4f}")
    print(f"Stats IA Raw -> Moyenne: {ai_mean:.4f}, Std: {ai_std:.4f}")
    
    # Project the model output back into the real space
    ...
    # Ground truth (translucent green)
    ...
    print(f"Aligned proof generated: {output_file}")
