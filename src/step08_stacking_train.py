...
from sobolev_loss import DerivativeLoss
...
def run_step8():
    ...
    # 1. Load the new Hybrid Mesh dataset
    ...
    target_cols = [
        ...
        'dV_drho_scaled',
        'dV_dvolvol_scaled',
        'dV_dalpha_scaled', # <-- critical fix applied here
        'dV_dF_scaled',
        'dV_dK_scaled'
    ]
    ...
    # ==============================
    print("Training the TabPFN oracle (on the train set)...")
    ...
    # ==============================
    print("TabPFN inference (chunking)...")
    ...
    # ==============================
    # 6. NORMALIZATION & MLOPS SAVE
    # ==============================
    mean = pfn_train.mean(axis=0)
    std = pfn_train.std(axis=0) + 1e-8 # safeguard against division by zero

    pfn_train = (pfn_train - mean) / std
    pfn_val = (pfn_val - mean) / std

    # ---> CRITICAL SAVE FOR STEP 9 <---
    ...
    print(f"TabPFN scaler saved: {scaler_path.name}")
    ...
    # ==============================
    print("Initializing the hybrid network...")
    ...
    # 10. SAVE
    # ==============================
    save_path = current_dir.parent / "models" / "tabpfn_sabr_step8_stacking.pth"
    torch.save(model.state_dict(), save_path)
    print(f"Model saved successfully: {save_path.name}")
