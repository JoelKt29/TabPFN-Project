...
def sabr_hagan_vol(K, F, T, alpha, beta, rho, volvol):
    """
    Hagan (2002) approximation formula for SABR implied volatility.
    Standard NumPy-compatible implementation.
    """
    eps = 1e-07
    logfk = np.log(F / K)
    fkbeta = (F * K)**(1 - beta)
    ...
    # Handle the ATM singularity (when K is very close to F)
    vol_atm = numerator / d
...
def plot_sabr_parameters():
    ...
    # Shared formatting for all plots
    for ax in axes.flat:
    ...
    plt.tight_layout()
    plt.savefig(Path(__file__).resolve().parent.parent / "figures" / "sabr_parameters_impact.png", dpi=300, bbox_inches='tight')
    print("Figure generated and saved as 'sabr_parameters_impact.png'")
    plt.show()
