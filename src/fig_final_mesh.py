...
# Import the exact mathematical formula
from step02_hagan_lognormal import Hagan2002LognormalSABR

# Chemins
current_dir = Path(__file__).resolve().parent
data_dir = current_dir.parent / "data"
config_path = current_dir / "ray_results" / "best_config.json"
model_path = current_dir.parent / "models" / "tabpfn_sabr_step8_stacking.pth"
...
# ==========================================
# MODEL DEFINITION (to avoid import conflicts)
# ==========================================
...
class TabPFNStackingModel(nn.Module):
    def __init__(self, config, n_outputs=7):
        super().__init__()
        # Force 11 to match Step 8 exactly
        input_dim = 11 
        layers = []
        prev_dim = input_dim
...
def generate_true_3d_proof():
    print("--- Generating 3D proof (real scale) ---")
    device = 'cuda' if torch.cuda.is_available() else ('mps' if torch.backends.mps.is_available() else 'cpu')

    # 1. Load configs and scalers
    ...
    # Scenario parameters (must lie within the Hybrid Mesh bounds)
    beta, rho, volvol, v_atm_n = 0.62, 0.0, 0.45, 0.027
    T, shift = 1.0, 0.0

    # 5. Compute the mathematical ground truth
    print("Computing the mathematical surface (Hagan)...")
    ...
    # 7. TabPFN inference + enrichment + normalization
    ...
    with torch.no_grad():
        ...
        # The skew (dV/dK) is the last column (index 6)
    ...
    print(f"Proof generated: {output_file}")
