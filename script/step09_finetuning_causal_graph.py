...
def train():
    ...
    criterion = AdaptiveDerivativeLoss(num_derivatives=num_derivatives).to(device)
    
    # Merge model parameters with learnable loss variables
    optimizer_params = list(model.parameters()) + list(criterion.parameters())
    optimizer = torch.optim.AdamW(optimizer_params, lr=config.get('lr'), weight_decay=1e-5)
