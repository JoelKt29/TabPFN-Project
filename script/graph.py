...
def show_tabpfn_derivative_noise():
    ...
    # 2. Setup Baseline TabPFN (Step 4 style)
    feature_cols = ['beta', 'rho', 'volvol', 'v_atm_n', 'alpha', 'F', 'K', 'log_moneyness']
    pfn_baseline = TabPFNRegressor(device='cpu', n_estimators=4)
    
    # Train on a small subset to simulate the prior
    ...
    # 4. Predict and compute Numerical Gradient
    pfn_preds = pfn_baseline.predict(test_batch)
    # The noise comes from here: numerical gradient of a non-smooth model
    pfn_derivative = np.gradient(pfn_preds, k_scaled)

    # 5. Get True Analytical Derivative for comparison
    true_derivative = (sample['dV_dK_scaled']) * np.ones_like(k_scaled) 

    # 6. Plotting
    plt.style.use('dark_background')
    fig, ax = plt.subplots(figsize=(12, 6))
    
    ax.plot(k_scaled, pfn_derivative, color='red', lw=1.5, label='TabPFN Numerical Derivative (Noisy Gradient)')
    ax.axhline(sample['dV_dK_scaled'], color='cyan', lw=3, label='Analytical Ground Truth (Step 5)')
